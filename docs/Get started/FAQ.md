...
Starting with fleetd 1.39.0, Fleet Desktop supports X11 and Wayland display sessions. On previous versions of fleetd
only X11 display sessions were supported. Ubuntu 21.04+ and Fedora 25+ come with Wayland enabled by default, to use X11 instead of Wayland you can set `WaylandEnable=false` in `/etc/gdm3/custom.conf` and reboot.
